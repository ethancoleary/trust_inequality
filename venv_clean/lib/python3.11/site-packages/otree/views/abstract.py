...
class FormPageOrInGameWaitPage:
    ...
    def render_page(self, context):
        from otree.pypage import render_pypage, get_pypage_path

        app_name = get_app_label_from_import_path(self.__module__)
        page_name = self.__class__.__name__
        if get_pypage_path(app_name, page_name):
            title_html, meta_title, content_html, debug_dict = render_pypage(
                app_name, page_name, self.player, context
            )
            context['_py_title'] = title_html
            context['_py_meta_title'] = meta_title
            context['_py_content'] = content_html
            if debug_dict and settings.DEBUG and hasattr(self, 'debug_tables'):
                self.debug_tables.append(
                    DebugTable(title='debug info', rows=debug_dict.items())
                )
            return render(f'otree/Py{self._template_type}.html', context)
        return render(
