...
def _merge_project_fields():
    """
    Allow PARTICIPANT_FIELDS / SESSION_FIELDS to be defined as a dict in project.py
    (the proprietary version uses dicts of column types). Here we only use the keys
    and treat them the same as the legacy list-of-strings format in settings.py.

    Called lazily from main.setup() to avoid a circular import: project.py typically
    imports from otree.api, which transitively imports otree.settings.
    """
    try:
        ...
        return
    g = globals()
    for setting_name in ('PARTICIPANT_FIELDS', 'SESSION_FIELDS'):
        project_fields = getattr(project_module, setting_name, None)
        if not project_fields:
            continue
        existing = list(g.get(setting_name, []))
        existing.extend(k for k in project_fields if k not in existing)
        g[setting_name] = existing
