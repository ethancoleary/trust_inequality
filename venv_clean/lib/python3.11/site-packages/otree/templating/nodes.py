...
def _render_formfield(fld, label):
...
class NextButton(Node):
    def wrender(self, context, label=None):
        if label is None:
            # Translators: the text of the 'next' button
            label = gettext('Next')
        return f'''
        <p>
            <button class="otree-btn-next btn btn-primary">{label}</button>
        </p>
        '''
